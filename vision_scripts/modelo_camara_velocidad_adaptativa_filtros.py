...
def detectar_bola(mask, img_rgb=None):
    """
    Devuelve 'radio', 'area', 'cx' y 'cy' si encuentra la bola. None en caso contrario.
    Usa EXCLUSIVAMENTE la Transformada de Hough para detectar la pelota, ignorando 
    completamente cv2.findContours y los momentos de la imagen.
    """
    if img_rgb is None:
        return None
        
    # Preprocesamiento (Crucial para que funcione bien)
    gris = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
    gris_desenfocado = cv2.medianBlur(gris, 5) # El número 5 ayuda a suavizar
    ...
    for c in circulos:
        ...
        # Validar que el centro no esté fuera de los límites de la imagen
        if cx < 0 or cx >= mask.shape[1] or cy < 0 or cy >= mask.shape[0]:
            continue
            
        # Comprobar si el centro del círculo detectado cae sobre un píxel rojo en la máscara
        if mask[cy, cx] > 0:
            area = float(np.pi * (radio ** 2))
            
            # Filtramos por tamaño y nos quedamos con el círculo válido más grande
            if area > max_area and area > AREA_MIN:
                max_area = area
                # Devolvemos el radio como primer elemento en lugar del 'contorno'
...
def dibujar_debug(img_rgb, mask, deteccion):
    ...
    # Volvemos a aplicar Hough solo para poder dibujar los círculos en la ventana de debug
    gris = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
    gris_desenfocado = cv2.medianBlur(gris, 5)
    circulos = cv2.HoughCircles(
        gris_desenfocado, 
        cv2.HOUGH_GRADIENT, 
        dp=1, 
    ...
    # Dibujamos todos los círculos encontrados por Hough en verde (grosor 2)
    if circulos is not None:
        ...
        for i in circulos[0, :]:
            ...
            cv2.circle(debug, centro, 2, (0, 0, 255), 2)

    # Dibujamos la detección final validada en morado (usando el radio de Hough)
...
def main(args=None):
    ...
    try:
        while coppelia.is_running():
            ...
            # Pasamos 'img' a detectar_bola para que el detector Hough pueda funcionar
            deteccion = detectar_bola(mask, img)
