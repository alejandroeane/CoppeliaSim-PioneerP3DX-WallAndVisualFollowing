...
class PID:
    ...
    def update(self, error, dt):
        ...
        # Filtro EMA para eliminar "tirones" y ruido del sensor en movimiento rápido
...
def wall_follow_step(readings, pid, state, dt, config=None):
    ...
    thresh_izq1_0 = config.get('THRESH_IZQ1_0', 0.25)
    thresh_izq1_1 = config.get('THRESH_IZQ1_1', 0.35)
    thresh_izq2_15 = config.get('THRESH_IZQ2_15', 0.35)
    thresh_izq2_14 = config.get('THRESH_IZQ2_14', 0.3)
    thresh_frontal = config.get('THRESH_FRONTAL', 0.4)
    thresh_lateral = config.get('THRESH_LATERAL', 0.25)
    ...
    thin_wall_1 = (min([readings[1], readings[2], readings[3], readings[4], readings[5]]) < 0.25)
    thin_wall_2 = (min([readings[6], readings[7]]) < 0.1)
    ...
    new_state = state
    giro_derecha_1 = (cond_1 and frontal and cond_2) 
    giro_derecha_2 = lateral_cond
    giro_derecha_3 = (frontal and not cond_1 and not cond_2)
    giro_derecha_4 = thin_wall_1
    giro_derecha = giro_derecha_1 or giro_derecha_2 or giro_derecha_3 or giro_derecha_4
    giro_izquierda = not cond_1 and cond_2 and not thin_wall_2
    if giro_derecha:
        new_state = State.AVOID_FRONT
        pid.reset()
        lspeed, rspeed = config.get('TURN_SPEED_POS', 1.7), config.get('TURN_SPEED_NEG', -0.4)

    elif not giro_derecha and giro_izquierda:
        new_state = State.TURN_TO_WALL
        pid.reset()
        lspeed, rspeed = config.get('TURN_SPEED_NEG', -0.4), config.get('TURN_SPEED_POS', 1.7)
